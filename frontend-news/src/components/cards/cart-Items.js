            ...
            <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2} py={1}>
              <Stack direction="row" alignItems="center" spacing={2}>
                ...
                <Box>
                  <Typography variant="subtitle1" noWrap>
                    {loading ? <Skeleton variant="text" width={160} /> : item.name.slice(0, 18)}
                  </Typography>
                  <Stack direction="row" gap={1}>
                    {loading ? (
                      <Skeleton variant="text" width={120} />
                    ) : item.type === 'simple' ? null : (
                      item.variantName.split('/').map((val, i) => (
                        <Typography key={val} variant="body2" sx={{ span: { textTransform: 'uppercase' } }}>
                          <b>{val}:</b> <span>{item.variant.split('/')[i]}</span>
                        </Typography>
                      ))
                    )}
                  </Stack>
              ...
              <Typography variant="subtitle1">
                {loading ? (
                  ...
                  <>{fCurrency(cCurrency(parseInt(item.subtotal)))}</>
                )}
              </Typography>
            </Stack>
