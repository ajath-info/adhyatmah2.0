...
export default function VendorServiceBookingPage() {
  ...
  //       setZipError('Invalid pincode or no address found');
  //       return;
  //     }
  ...
  //   } catch (error) {
  //     setZipError('Failed to fetch pincode details');
  //   } finally {
  //     setZipLoading(false);
  //   }
  // };
  ...
  const poojaKitTotal = !poojaKitSkipped ? POOJA_KIT_PRICE * poojaKitCount : 0;
  const instantKitTotal = !instantKitSkipped ? INSTANT_KIT_PRICE * instantKitCount : 0;
                ...
                {!poojaKitSkipped ? (
                  <>
                    <Box sx={{ borderRadius: 2, overflow: 'hidden' }}>
                      {/* Image Section */}
                      <Box sx={{ width: '100%', height: 240, bgcolor: 'rgba(249,115,22,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  ...
                  /* Skipped state — with undo */
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 1.5, py: 1, borderRadius: 1.5, bgcolor: 'action.hover', border: '1px dashed', borderColor: 'divider' }}>
                    <Typography variant="caption" sx={{ color: '#888' }}>Pooja Kit skipped</Typography>
                    <Typography
                      variant="caption"
                      onClick={() => setPoojaKitSkipped(false)}
                      sx={{ color: '#f97316', cursor: 'pointer', textDecoration: 'underline' }}
                    >
                      Undo
                    </Typography>
                  </Box>
              ...
              <CardContent>
                ...
                {!instantKitSkipped ? (
                  <>
                    <Box sx={{ borderRadius: 2, overflow: 'hidden' }}>
                      {/* Image Section */}
                      <Box sx={{ width: '100%', height: 240, bgcolor: 'rgba(99,102,241,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                  ...
                  /* Skipped state — with undo */
                  <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 1.5, py: 1, borderRadius: 1.5, bgcolor: 'action.hover', border: '1px dashed', borderColor: 'divider' }}>
                    <Typography variant="caption" sx={{ color: '#888' }}>Instant Kit skipped</Typography>
                    <Typography
                      variant="caption"
                      onClick={() => setInstantKitSkipped(false)}
                      sx={{ color: '#6366f1', cursor: 'pointer', textDecoration: 'underline' }}
                    >
                      Undo
                    </Typography>
                  </Box>
              ...
              <CardContent>
                    ...
                    {timeSlots.map((slot) => {
                      ...
                      return (
                        <Box
                          ...
                          sx={{
                            px: 1.5, py: 0.75, borderRadius: 1.5, border: '1.5px solid',
                            borderColor: isSelected ? 'primary.main' : 'divider',
                            bgcolor: isSelected ? 'primary.main' : 'background.paper',
                            color: isSelected ? '#fff' : 'text.primary',
                            cursor: 'pointer', fontSize: '0.75rem', fontWeight: isSelected ? 600 : 400,
                            userSelect: 'none', transition: 'all 0.15s ease', whiteSpace: 'nowrap',
                            '&:hover': { borderColor: 'primary.main', bgcolor: isSelected ? 'primary.dark' : 'primary.50', color: isSelected ? '#fff' : 'primary.main' }
                          }}
                        >
                          {slot.label}
                        </Box>
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField
                      fullWidth label="Zip Code" value={zip}
                      onChange={(e) => {
                    ...
                    {instantKitCount > 0 && !instantKitSkipped && (
                      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ py: 0.75 }}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <Typography variant="body2" sx={{ color: '#555' }}>
                            Instant Kit{' '}
                            <Typography component="span" variant="caption" sx={{ color: '#6366f1' }}>(×{instantKitCount})</Typography>
                          </Typography>
                        </Stack>
      ...
      {paymentDialogOpen && selectedDate && selectedTime && streetAddress && city && district && state && country && (
        <BookingPaymentDialog
          ...
          bookingData={{
            ...
            totalAmount: service.price, // ✅ FIX: sirf service price, addons alag hain
            addons: {
              poojaKit: poojaKitCount > 0 && !poojaKitSkipped ? { name: poojaKit.name, price: POOJA_KIT_PRICE, quantity: poojaKitCount } : null,
              instantKit: instantKitCount > 0 && !instantKitSkipped ? { name: instantKit.name, price: INSTANT_KIT_PRICE, quantity: instantKitCount } : null
            }
          }}
