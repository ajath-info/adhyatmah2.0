...
const Search = async (req, res) => {
  try {
    ...
    const currentPandit = shop ? await User.findById(shop).select(["_id"]) : "";
    ...
    if (shop && !currentPandit) {
      return res
        .status(404)
        .json({ success: false, message: "Invalid pandit ID" });
    }
    ...
    const products = await Products.aggregate([
      {
        $match: {
          $and: [
            ...
            { status: "published" },
            ...(currentPandit ? [{ shop: currentPandit.shop }] : []),
...
const getFilters = async (req, res) => {
  try {
    ...
    const pandits = await User.find({ role: "vendor" }).select([
      "_id",
      "firstName",
      "lastName",
    ]);
